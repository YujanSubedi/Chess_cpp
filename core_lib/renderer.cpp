...
void Chess_Renderer::Render_for_white(GAME_BOARD CHESS_GAME_BOARD){
	...
	if(selected_piece != none_piece){
		DrawRectangle(82+selected_pos[1]*80, 82+(7-selected_pos[0])*80, 78, 78, (Color){30, 150, 30, 255});
		for(MOVE_CLASS highligeted_move: temp_move){
			if(highligeted_move.type==movement || highligeted_move.type==promotion){
				DrawRectangle(82+highligeted_move.final_pos[1]*80, 82+(7-highligeted_move.final_pos[0])*80, 78, 78, (Color){30, 150, 150, 128});
			}
			else if(highligeted_move.type==capture || highligeted_move.type==promotion_with_capture){
				DrawRectangle(82+highligeted_move.final_pos[1]*80, 82+(7-highligeted_move.final_pos[0])*80, 78, 78, (Color){150, 30, 30, 128});
			}
		}
	}
	for(int i=0; i<12; i++){
		for(array<int, 2> piece_position: CHESS_GAME_BOARD.get_Pieces_positions(piece_index(i))){
			DrawTexture(Pieces_textures[i], x_offset + size_of_square*piece_position[1], y_offset + size_of_square*(7-piece_position[0]), GRAY);
		}
	}
}
...
void Chess_Renderer::Render_for_black(GAME_BOARD CHESS_GAME_BOARD){
	...
	if(selected_piece != none_piece){
		DrawRectangle(82+(7-selected_pos[1])*80, 82+(selected_pos[0])*80, 78, 78, (Color){30, 150, 30, 255});
		for(MOVE_CLASS highligeted_move: temp_move){
			if(highligeted_move.type==movement || highligeted_move.type==promotion){
				DrawRectangle(82+(7-highligeted_move.final_pos[1])*80, 82+(highligeted_move.final_pos[0])*80, 78, 78, (Color){30, 150, 150, 128});
			}
			else if(highligeted_move.type==capture || highligeted_move.type==promotion_with_capture){
				DrawRectangle(82+(7-highligeted_move.final_pos[1])*80, 82+(highligeted_move.final_pos[0])*80, 78, 78, (Color){150, 30, 30, 128});
			}
		}
	}
	for(int i=0; i<12; i++){
		for(array<int, 2> piece_position: CHESS_GAME_BOARD.get_Pieces_positions(piece_index(i))){
			DrawTexture(Pieces_textures[i], x_offset + size_of_square*(7-piece_position[1]), y_offset + size_of_square*(piece_position[0]), GRAY);
		}
	}
}


bool Chess_Renderer::Handle_mouse_click_white(GAME_BOARD &CHESS_GAME_BOARD, MOVE_CLASS &User_move){
	clicked_pos[0] = (8-GetMouseY()/80);
	clicked_pos[1] = GetMouseX()/80-1;
	if((clicked_pos[0]>=0 && clicked_pos[0]<8) && (clicked_pos[1]>=0 && clicked_pos[1]<8)){
		...
		else if(selected_piece == none_piece || (clicked_color == selected_color)){
...
bool Chess_Renderer::Handle_mouse_click_black(GAME_BOARD &CHESS_GAME_BOARD, MOVE_CLASS &User_move){
