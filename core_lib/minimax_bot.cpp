...
int Minimax_bot::minimax_evalutaion(GAME_BOARD Given_board,
                                    MOVE_CLASS simulated_move, int alpha_value,
                                    int beta_value, unsigned depth) {
  ...
  vector<MOVE_CLASS> Possible_moves_by_bot =
      Given_board.get_all_available_moves(Given_board.Get_turn());
  int eval;
  for (MOVE_CLASS temp_move : Possible_moves_by_bot) {
    eval = minimax_evalutaion(Given_board, temp_move, alpha_value, beta_value,
                              depth + 1);
...
MOVE_CLASS Minimax_bot::get_best_move(GAME_BOARD Given_board) {
  ...
  int eval;
  for (MOVE_CLASS temp_move : Possible_moves_by_bot) {
    future_evaluations.push_back(
        std::async(std::launch::async, &Minimax_bot::minimax_evalutaion, this,
                   Given_board, temp_move, -1000, 1000, 0));
    // eval = minimax_evalutaion(Given_board, temp_move, 0);
  }
  for (int i = 0; i < Possible_moves_by_bot.size(); i++) {
    eval = future_evaluations[i].get();
    if ((selected_color == white_colored && eval > best_eval) ||
        (selected_color == black_colored && eval < best_eval)) {
      best_eval = eval;
...
void Minimax_bot::play_in_parallel(GAME_BOARD &CHESS_GAME_BOARD) {
  ...
  if (can_create_thread_for_bot) {
    Future_bot_move =
        std::async(std::launch::async, &Minimax_bot::get_best_move, this,
                   CHESS_GAME_BOARD);
    can_create_thread_for_bot = false;
    //(&GAME_BOARD::play_move, &CHESS_GAME_BOARD,
    //MOVE_BY_BOT(CHESS_GAME_BOARD));
