...
CHESS_BITBOARD::CHESS_BITBOARD()
    : whitepawn_mask(0x000000000000ff00), blackpawn_mask(0x00ff000000000000),
      leftmost_mask(0x7f7f7f7f7f7f7f7f), rightmost_mask(0xfefefefefefefefe),
...
void CHESS_BITBOARD::update() {
  ...
  targets_bitboards[0] =
      pawn_targets(piece_bitboards[white_pawn], white_colored);
  targets_bitboards[1] =
      pawn_targets(piece_bitboards[black_pawn], black_colored);
...
vector<array<int, 2>>
CHESS_BITBOARD::get_all_positions(piece_index Piece_type) {
...
vector<array<int, 2>>
CHESS_BITBOARD::get_bitboard_positions(unsigned long passed_bitboard) {
  vector<array<int, 2>> Positions_of_piece;
  unsigned long masking_bit = 1;
  for (int i = 0; i < 8; i++) {
    for (int j = 0; j < 8; j++) {
...
unsigned long CHESS_BITBOARD::pawn_targets(unsigned long pawn_bitboard,
...
unsigned long CHESS_BITBOARD::pawn_movements(unsigned long pawn_bitboard,
                                             Piece_color selected_color) {
  unsigned long all_moves = 0;
...
void CHESS_BITBOARD::update_move(MOVE_CLASS played_move) {
  if (played_move.type == movement) {
    ...
    set_bit(played_move.initial_piece, played_move.final_pos);
  } else if (played_move.initial_piece == white_pawn ||
             played_move.initial_piece == black_pawn) {
...
piece_index CHESS_BITBOARD::Get_piece_type(array<int, 2> position) {
  ...
  unsigned long masking_bits = 1;
  masking_bits <<= (position[0] * 8 + position[1]);
  if (masking_bits & unoccupied_bitboard)
    return none_piece;
  for (int i = 0; i < 12; i++) {
...
vector<MOVE_CLASS> CHESS_BITBOARD::get_moves(array<int, 2> position) {
  ...
  if (selected_piece_type == none_piece)
    return result_vector;
  Piece_color Selected_color =
      (selected_piece_type & 1) ? black_colored : white_colored;
  ...
  if (selected_piece_type == white_pawn || selected_piece_type == black_pawn) {
    available_position_bitboard &=
        pawn_movements(position_mask, Selected_color);
    capture_position_bitboard &= pawn_targets(position_mask, Selected_color);
    if ((Selected_color == white_colored && position_mask & blackpawn_mask) ||
        (Selected_color == black_colored && position_mask & whitepawn_mask)) {
  ...
  if (selected_piece_type == white_knight ||
      selected_piece_type == black_knight) {
    temp_bitboard = knight_moves(position_mask);
  } else if (selected_piece_type == white_bishop ||
             selected_piece_type == black_bishop) {
    temp_bitboard = bishop_moves(position_mask);
  } else if (selected_piece_type == white_rook ||
             selected_piece_type == black_rook) {
    temp_bitboard = rook_moves(position_mask);
  } else if (selected_piece_type == white_queen ||
             selected_piece_type == black_queen) {
    temp_bitboard = queen_moves(position_mask);
  } else if (selected_piece_type == white_king ||
             selected_piece_type == black_king) {
...
bool CHESS_BITBOARD::is_move_valid(MOVE_CLASS given_move) {
  ...
  Piece_color player_color =
      given_move.initial_piece & 1 ? black_colored : white_colored;
  undo_move(given_move);
  if ((next_game_state == Invalid_state) ||
      (next_game_state == Black_checked && player_color == black_colored) ||
      (next_game_state == White_checked && player_color == white_colored)) {
    return false;
  }
  return true;
}
